...
struct MyGraphChiProgram : public GraphChiProgram<VertexDataType, EdgeDataType> {
	...
	*/
	void update(graphchi_vertex<VertexDataType, EdgeDataType> &vertex, graphchi_context &gcontext) {
		if (final_run) {
			int this_iteration = (gcontext.iteration - iteration_num - 1) % 3;
			if (this_iteration % 3 == 0) {
			...
			else if (this_iteration % 3 == 1) {				
				gcontext.scheduler->add_task(0);
				node_data now_node = vertex.get_data();
				pthread_mutex_lock(&my_mutex);
				node_transfer.insert(now_node);
				pthread_mutex_unlock(&my_mutex);
				if (now_node.add_flag == gcontext.iteration - 1) return;
				for (int i=0; i<vertex.num_inedges(); i++) {
					gcontext.scheduler->add_task(vertex.inedge(i)->vertex_id());
					pthread_mutex_lock(&my_mutex2);
					if (vertex.inedge(i)->vertex_id()>1 && vertex.id() > 1) {
			...
			if (gcontext.iteration == 0) {
				...
				int this_iteration = (gcontext.iteration - 2) % 2;
				if (this_iteration == -1) this_iteration += 2;
	...
	void after_iteration(int iteration, graphchi_context &gcontext) {
		...
		if (iteration != 0) {
			if (final_run == true) {
				...
				iteration_num = gcontext.iteration;
				int this_iteration = (gcontext.iteration - 2) % 2;
				if (this_iteration == -1) this_iteration += 2;
				if (this_iteration % 2 == 1) {
...
void * dynamic_graph_reader(void * info) {
	...
	if (cnt % batch_num == 0) {
		...
		engine->add_task(0);
				...
				logstream(LOG_DEBUG) << 1 << std::endl;
	pthread_barrier_wait(&graph_barrier);
				logstream(LOG_DEBUG) << 2 << std::endl;
	pthread_barrier_wait(&start_barrier);
	final_run = true;
	vector<int> cand;
	cand.clear();
	for (int i=2; i<engine->num_vertices(); i++)
		cand.push_back(i);
	//shuffle(cand);
	logstream(LOG_DEBUG) << iteration_num << std::endl;
	for (int i=0; i<engine->num_vertices()-2; i++) {
		engine->add_task(cand[i]);
		if (i == 0) continue;
		if ((i % train_batch == 0) || (i == engine->num_vertices()-3)) {
			pthread_barrier_wait(&graph_barrier);
			pthread_barrier_wait(&start_barrier);
		}
	}
	//engine->add_task(i);
	logstream(LOG_DEBUG) << 123 << std::endl;
	pthread_barrier_wait(&graph_barrier);
...
int main(int argc, const char ** argv) {
	...
	batch_num = 200000;
	train_batch = get_option_int("batch");
